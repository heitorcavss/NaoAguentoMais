...
int lerOperadorR2PorParsing(Matriz *m){
    ...
    printf("\nDigite o operador T: R^2 -> R^2 por coordenadas.\n");
    printf("Exemplo: T(x,y) = (x - y, y)\n");
    printf("Coordenada 1: x - y\n");
    printf("Coordenada 2: y\n\n");

    for(int i = 0; i < 2; i++){
        printf("Coordenada %d: ", i + 1);

        if(fgets(linha, sizeof(linha), stdin) == NULL){
            printf("Erro ao ler coordenada.\n");
            return 0;
        }

        if(!parseExpressaoLinear(linha, variaveis, 2, m->valores[i])){
